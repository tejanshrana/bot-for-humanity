...
```bash
bot-for-humanity$ sam build --use-container
```

The SAM CLI installs dependencies defined in `bot_for_humanity/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
bot-for-humanity$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
bot-for-humanity$ sam logs -n HelloWorldFunction --stack-name bot-for-humanity --tail
```
...
```bash
bot-for-humanity$ pip install -r tests/requirements.txt --user
# unit test
bot-for-humanity$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
bot-for-humanity$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name bot-for-humanity
```
